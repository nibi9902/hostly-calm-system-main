...
export default function Precios() {
  ...
  const faqs = t('precios_din.faqs', { returnObjects: true }) as FaqItem[];
  const features = t('precios_din.features', { returnObjects: true }) as FeatureItem[];

  return (
    <PageShell
      title={t('precios_din.meta_title')}
      description={t('precios_din.meta_description')}
      path="/funciones/precios"
      schemas={[
        faqPageSchema(faqs),
        breadcrumbSchema([
          { name: "Hostly", url: "/" },
          { name: t('precios_din.breadcrumb_funciones'), url: "/funciones/precios" },
          { name: t('precios_din.breadcrumb_name'), url: "/funciones/precios" },
        ]),
      ]}
    >
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }}>
            <h1 className="text-4xl md:text-6xl font-bold text-[#0f172a] tracking-tight mb-6 leading-tight">
              {t('precios_din.h1_line1')}<br />{t('precios_din.h1_line2')}
            </h1>
            <p className="text-lg md:text-xl text-slate-500 max-w-2xl mx-auto mb-10 leading-relaxed">
              {t('precios_din.subtitle')}
            </p>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-20 bg-[#f8fafc]">
        <div className="max-w-4xl mx-auto">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#1a3a8f] mb-4">{t('precios_din.sol_eyebrow')}</p>
          <h2 className="text-3xl md:text-4xl font-bold text-[#0f172a] tracking-tight mb-10">
            {t('precios_din.sol_h2')}
          </h2>
