...
export default function Pagos() {
  ...
  const faqs = t('pagos.faqs', { returnObjects: true }) as FaqItem[];
  const features = t('pagos.features', { returnObjects: true }) as FeatureItem[];

  return (
    <PageShell
      title={t('pagos.meta_title')}
      description={t('pagos.meta_description')}
      path="/funciones/pagos"
      schemas={[
        faqPageSchema(faqs),
        breadcrumbSchema([
          { name: "Hostly", url: "/" },
          { name: t('pagos.breadcrumb_funciones'), url: "/funciones/pagos" },
          { name: t('pagos.breadcrumb_name'), url: "/funciones/pagos" },
        ]),
      ]}
    >
      ...
      <section className="py-20 px-6 md:px-12 lg:px-20 bg-[#f8fafc]">
        <div className="max-w-4xl mx-auto">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#1a3a8f] mb-4">{t('pagos.sol_eyebrow')}</p>
      ...
      <section className="py-10 px-6 md:px-12 lg:px-20">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.15em] text-[#1a3a8f]/60 mb-1">{t('pagos.finanzas_eyebrow')}</p>
              <p className="font-bold text-[#0f172a] text-base group-hover:text-[#1a3a8f] transition-colors">
                {t('pagos.finanzas_title')}
              </p>
              <p className="text-sm text-slate-500 mt-0.5">{t('pagos.finanzas_desc')}</p>
            </div>
