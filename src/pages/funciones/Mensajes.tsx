...
export default function Mensajes() {
  ...
  const faqs = t('mensajes.faqs', { returnObjects: true }) as FaqItem[];
  const features = t('mensajes.features', { returnObjects: true }) as FeatureItem[];

  return (
    <PageShell
      title={t('mensajes.meta_title')}
      description={t('mensajes.meta_description')}
      path="/funciones/mensajes"
      schemas={[
        faqPageSchema(faqs),
        breadcrumbSchema([
          { name: "Hostly", url: "/" },
          { name: t('mensajes.breadcrumb_funciones'), url: "/funciones/mensajes" },
          { name: t('mensajes.breadcrumb_name'), url: "/funciones/mensajes" },
        ]),
      ]}
    >
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }}>
            <h1 className="text-4xl md:text-6xl font-bold text-[#0f172a] tracking-tight mb-6 leading-tight">
              {t('mensajes.h1_line1')}<br />{t('mensajes.h1_line2')}
            </h1>
      ...
      <section className="py-20 px-6 md:px-12 lg:px-20 bg-[#f8fafc]">
        <div className="max-w-4xl mx-auto">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#1a3a8f] mb-4">{t('mensajes.sol_eyebrow')}</p>
